...
아니죠? 오히려 우하향 그래프를 그리고 있습니다. 대부분 현실입니다.   
...
### Devops철학과 원칙    
DevOps 철학과 원칙
Kubernetes의 핵심 3대 철학:
- Immutable
  - Kubernetes를 만들면서 시스템의 불변성을 지키고자 한 것이 핵심이다.  
컨테이너와 리소스는 한 번 생성되면 변경하지 않고, 필요 시 새로 배포하는 방식으로 운영한다.  
- Self-healing
  - 시스템에 문제가 생기더라도 빠르게 복구할 수 있어야 한다.
누락된 요소나 장애가 발생했을 때, Kubernetes가 자동으로 상태를 정상으로 유지하도록 설계되었다.   
